...
def _save_as_json(daily_power_data: dict, country: str, timespan: int):
    file_name = f"{TODAY.strftime('%Y-%m-%d')}_{timespan}_({country}).json"
    result_file = RESULT_PATH / file_name
    RESULT_PATH.mkdir(exist_ok=True)
    file_content = json.dumps(daily_power_data, indent=4, sort_keys=False)
...
if __name__ == "__main__":
    ...
    daily_power = _get_public_power(
        country=arguments.country,
        timespan=int(arguments.days)
    )
    _save_as_json(
        daily_power_data=daily_power,
        country=arguments.country,
        timespan=int(arguments.days)
    )
