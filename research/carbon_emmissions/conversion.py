...
def estimate_carbon_emmissions(
    power_w: list[int], gco2_per_kwh: list[int], unix_seconds: list[int]
) -> float:
    """Estimate the carbon emmissions

    Args:
        power_w (list[int]): Data points for power (in Watts).
        gco2_per_kwh (list[int]): Data points for carbon intensity (in gCO2-eq. / kWh).
    ...
    gco2_per_kwh_interp = interpolate_by_minutes(
        data=gco2_per_kwh, unix_seconds=unix_seconds
    )
    ...
    for index, power in enumerate(power_w_interp):
        ...
        emmissions = kwh * gco2_per_kwh_interp[index]
        gco2eq += emmissions
    return gco2eq
